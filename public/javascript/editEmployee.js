...
$(document).ready(() => {
    ...
    // // unbinding existing click event
    // $("#saveEmployee").unbind();

    // Creating new binding for saving employee
    ...
    // Setting primary positions
    employee.primaryPositions.forEach(position => {
        $("#addPrimaryPosition").click();
        $(".primaryPositions:first").val(position);
    })

    // Setting secondary positions
    employee.secondaryPositions.forEach(position => {

        $("#addSecondaryPosition").click();
        $(".secondaryPositions:first").val(position);
    })

    // Setting preferred shifts
    daysOfWeek.forEach(day => {
        employee.preferredShifts[day].forEach(shift => {
            contentId = day + "_" + shift.position.split(" ").join("+") + "_" + shift.startTime + "_" + shift.endTime;

            $('[id="' + contentId + '"]').attr("checked", true);
        })
    })

    // Setting day unavailability
    employee.daysUnavailable.forEach(obj => {
        $("#addDayUnavailability").click();
        $(".dayUnavailable:first").val(capitalize(obj.dayOfWeek));
        $(".dayUnavailabilityStart:first").val(obj.startTime);
        $(".dayUnavailabilityEnd:first").val(obj.endTime);

    })

    // Setting date unavailability
    employee.datesUnavailable.forEach(obj => {
        $("#addDateUnavailability").click();
        $(".dateUnavailable:first").val(obj.date);
        $(".dateUnavailableStart:first").val(obj.startTime);
    ...
    employee.prolongedUnavailability.forEach(obj => {
        ...
        $(".prolongedUnavailabilityEnd:first").val(obj.endTime);
    })
});
